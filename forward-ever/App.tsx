...
import { bringToLife, ImageFile } from './services/gemini';
...
const App: React.FC = () => {
  ...
  const handleGenerate = async (promptText: string, files?: File[]) => {
    ...
    try {
      ...
      const html = await bringToLife(promptText, processedImages);
      
      if (html) {
        const newCreation: Creation = {
          id: crypto.randomUUID(),
          name: files && files.length > 0 ? (files.length > 1 ? `${files[0].name} +${files.length - 1}` : files[0].name) : 'Heritage Vision',
          html: html,
          // Store just the first image for the thumbnail to save storage space
          originalImage: processedImages.length > 0 ? `data:${processedImages[0].mimeType};base64,${processedImages[0].data}` : undefined,
          timestamp: new Date(),
        };
      ...
      alert("Something went wrong while bringing your vision to life. Please try again.");
    } finally {
