...
import { HomeIcon, MapIcon, PhotoIcon, ClipboardDocumentListIcon } from '@heroicons/react/24/outline';
import { HomeModernIcon, BuildingStorefrontIcon, SparklesIcon, GlobeAmericasIcon } from '@heroicons/react/24/solid';
      ...
      <div className={`relative w-20 h-28 md:w-28 md:h-40 rounded-lg backdrop-blur-md transition-all duration-1000 ${stage === 2 ? 'bg-zinc-800/40 border-zinc-500/50 shadow-xl scale-110 -translate-y-4' : 'bg-zinc-900/10 border-zinc-800 scale-100 border border-dashed'}`}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <div className={`absolute transition-all duration-700 flex flex-col items-center ${stage === 2 ? 'opacity-100 scale-100 blur-0' : 'opacity-0 scale-75 blur-sm'}`}>
             <FinalIcon className="w-10 h-10 md:w-14 md:h-14 text-emerald-500" />
             {stage === 2 && (
               <div className="mt-3 flex items-center gap-2 px-2 py-1 bg-zinc-900/80 rounded-full border border-zinc-700/50">
                 ...
                 <div className="w-8 h-0.5 bg-zinc-700 rounded-full overflow-hidden">
                    <div className="h-full bg-emerald-500 w-2/3 animate-[pulse_1s_infinite]"></div>
                 </div>
               </div>
             )}
          </div>
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        {/* Top Left: Old House -> New Vision */}
        <div className="hidden lg:block">
            <DrawingTransformation 
            initialIcon={HomeIcon} 
            finalIcon={HomeModernIcon} 
            label="RESTORE"
            delay={0} 
            x="4%" 
            y="8%"
        ...
        {/* Bottom Right: Land Map -> Resort */}
        <div className="hidden md:block">
            <DrawingTransformation 
            initialIcon={MapIcon} 
            finalIcon={BuildingStorefrontIcon} 
            label="DEVELOP"
            delay={3000} 
            x="88%" 
            y="75%"
        ...
        {/* Top Right: Survey/Doc -> Master Plan */}
        <div className="hidden lg:block">
            <DrawingTransformation 
            initialIcon={ClipboardDocumentListIcon} 
            finalIcon={GlobeAmericasIcon} 
            label="HERITAGE"
            delay={6000} 
            x="88%" 
            y="12%"
      ...
      <div className="text-center relative z-10 max-w-6xl mx-auto px-4 pt-8">
        <div className="inline-block mb-4 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-mono tracking-wider uppercase">
          Architectural Evolution & Heritage Visualization
        </div>
        <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-bold tracking-tighter text-white mb-6 leading-[1.1]">
          Forward <span className="text-emerald-500">Ever</span>,<br/>
          Backward <span className="underline decoration-4 decoration-zinc-700 underline-offset-4 md:underline-offset-8 text-zinc-400">Never</span>.
        </h1>
        <p className="text-base sm:text-xl text-zinc-400 max-w-2xl mx-auto leading-relaxed font-light">
          Upload photos of your property, sketches of your dream retreat, or old family maps. 
          Watch as we transform your vision into a living, breathing reality.
        </p>
      </div>
    </>
  );
